...
//	p_toc0_cert += sizeof(TOC0_COMST_MAIN_SEQUENCE);*/
	...
	if (toc0_corrupted_type == BAD_SIGN) {
		char tmp_value[1024];
		memcpy(tmp_value, hash_of_cert, 32);
		hash_of_cert[2] += 0xA5;
		printf("------------------------------------\n");
		printf("generate toc0 with bad sign:\n");
		sunxi_dump(hash_of_cert, 32);
		printf("\n");
		printf("which should had been\n");
		sunxi_dump(tmp_value, 32);
		printf("------------------------------------\n");
	}
	/*	sunxi_dump(hash_of_cert, 32);
	//字符转换为整型*/
...
int create_cert_for_toc0(char *lpCfg, toc_descriptor_t *toc0, char *keypath)
{
	...
	switch (toc0_corrupted_type) {
		char tmp_value[1024];
		memset(tmp_value, 0, 1024);
	case BAD_HASH:
		memcpy(tmp_value, hash_value, 32);
		hash_value[2] += 0xA5;
		printf("--------------------------\n");
		printf("generate toc0 with bad hash:\n");
		sunxi_dump(hash_value, 32);
		printf("\n");
		printf("which should had been\n");
		sunxi_dump(tmp_value, 32);
		...
		break;
	case BAD_PK:
		memcpy(tmp_value, key_n, 560);
		printf("--------------------------\n");
		key_n[2] = ((key_n[2] >= '0') && (key_n[2] <= '9')) ? 'A' : '5';
		printf("generate toc0 with bad pk:%s\n", key_n);
		printf("\n");
		printf("which should had been:%s\n", tmp_value);
		printf("--------------------------\n");
		break;
	case BAD_MOD:
		printf("--------------------------\n");
		memcpy(tmp_value, key_e, 16);
		key_e[2] = ((key_e[2] >= '0') && (key_e[2] <= '9')) ? 'A' : '5';
		printf("generate toc0 with bad mod:%s\n", key_e);
		printf("\n");
		printf("which should had been:%s\n", tmp_value);
		printf("--------------------------\n");
		break;
