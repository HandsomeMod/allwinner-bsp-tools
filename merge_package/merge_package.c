...
int main(int argc, char *argv[])
{
	...
	int logic_start = 512 << 10;
	int uboot_start = 24 << 10;
	...
	debug("uboot_start: %dK\n", uboot_start / 1024);

	if (0 == strlen(boot0_file) || 0 == strlen(boot1_file) ||
	    0 == strlen(mbr_file) || 0 == strlen(partition_file) ||
	...
	image_cfg[1].offset = uboot_start;
	...
	for (i = 0; i < 3; i++) {
		ret = load_file_to_buffer(image_cfg[i].filename,
					  (void **)&file_buffer, &file_size);
		if (ret != 0) {
			goto __err_out;
		}
		memcpy(out_buffer + image_cfg[i].offset, file_buffer,
		       file_size);
		if (file_buffer) {
	...
	total_part_file_size =
	    load_file_from_partition_config(out_buffer + offset, file_buffer);
	if (file_buffer) {
		free_file_buffer(file_buffer);
		file_buffer = NULL;
	}
...
int load_file_from_partition_config(void *out_buffer, void *partition_cfg)
{
	...
	while (1) {
		...
		if (!script_parser_fetch_mainkey_sub("name", part_handle,
						     value)) {
			debug("part name=%s\n", (char *)value);
			memset(fullname, 0, sizeof(fullname));
			memset(filename, 0, sizeof(filename));
			ret = script_parser_fetch_mainkey_sub(
			    "downloadfile", part_handle, (int *)fullname);
			if (!ret) {
				strcpy(filename, (char *)fullname);
				debug("file name:%s\n", filename);
			}
			ret = script_parser_fetch_mainkey_sub(
			    "size", part_handle, (int *)value);
			if (!ret) {
				part_size = value[0];
			...
			if (strlen(filename)) {
				ret = load_file_to_buffer(
				    filename, (void **)(&partfile_buffer),
				    &file_size);
				if (ret == 0) {
					if (file_size > part_size) {
						printf("error: file size of "
						       "%s(%d) > part "
						       "size(%d)\n",
						       filename, file_size,
						       part_size);
