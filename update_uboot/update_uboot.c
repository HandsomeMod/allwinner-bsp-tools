...
/*
 * check is uboot_version newer than compare_version
 * return
 * 1:uboot_version newer than or equal to compare_version
 * 0:older
 */
static uboot_version_newer(char *uboot_version, char *compare_version)
{
	/*
	 * version comes in format "4.0.0", double quote not counted
	 *                    [i] = 0 2 4
	 */
	int i;

	/*printf("uboot_version:%s\t", uboot_version);*/
	/*printf("compare_version:%s\t", compare_version);*/
	for (i = 0; i < 5; i += 2) {
		if ((uboot_version[i] < '0') || ('9' < uboot_version[i])) {
			return 0;
		}
		if ((compare_version[i] < '0') || ('9' < compare_version[i])) {
			return 0;
		}
		if (uboot_version[i] > compare_version[i]) {
			/*printf("newer\n");*/
			return 1;
		} else if (uboot_version[i] < compare_version[i]) {
			/*printf("older\n");*/
			return 0;
		}
	}
...
int update_for_uboot(char *uboot_name)
{
	...
	if (uboot_version_newer(uboot_head->boot_head.version, "4.0.0")) {
		uboot_400_head = (struct extend_boot_head_400_t
					  *)((uint8_t *)uboot_head +
					     sizeof(struct spare_boot_head_t));
		if (!script_parser_fetch("platform", "debug_mode",
					 value)) {
			uboot_400_head->debug_mode = value[0];
		} else {
			uboot_400_head->debug_mode = 8;
		}
		printf("debug_mode =%d\n", uboot_400_head->debug_mode);
		if (!script_parser_fetch("target", "burn_secure_mode",
					 value)) {
			uboot_400_head->secure_mode = value[0];
		}
		printf("secure_mode =%d\n", uboot_400_head->secure_mode);
	}
